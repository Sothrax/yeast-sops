...
Stáhnout denní pohyby z ČSOB účtu `CZ65 0300 0000 0001 2345 6789`, naimportovat je do FlexiBee
(společnost `TAF_ESTATE_SRO`), provést primární spárování proti otevřeným
pohledávkám/závazkům a eskalovat nepárované položky nad limit `10000 CZK`
do kanálu `slack:#taf-accounting`.
...
- [ ] FlexiBee instance `TAF_ESTATE_SRO` dostupná
...
- HTTP 4xx (auth) → STOP, notify `slack:#taf-accounting`, open issue "ČSOB auth failed"
...
for tx in txs:
    if flexibee.query(
        evidence="banka",
        filter=f"varSym='{tx.payment_id}' and typPohybuK='in' and bankUcet='CZ65 0300 0000 0001 2345 6789'",
    ):
...
Pokud existují transakce s `abs(amount) >= 10000` a `matched=false`:

- Pošli do `slack:#taf-accounting` strukturovanou zprávu s:
...
- Notifikuj do `slack:#taf-accounting` s přímým odkazem
...
Pošli do `slack:#taf-accounting` shrnutí:

```
✅ Bank reconciliation ČSOB — TAF_ESTATE_SRO
...
- Slack report do `slack:#taf-accounting`
